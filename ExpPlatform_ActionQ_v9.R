...
setClass("ConcurrentTrtsData",
         representation(TrtAllos="data.frame",
                        Outcomes="numeric",TimesToOutcomes="OptionalNumeric"),
...
setClass("PatData",
         representation(ID="OptionalNumeric",BaseChars="OptionalNumeric",
                        ConcurrentTrtsDataList="list",PatTimes="OptionalNumeric"),
         prototype=list(ID=NULL,BaseChars=NULL,PatTimes=NULL),
         validity=function(object){
           if (length(object@ConcurrentTrtsDataList)==0) TRUE
           else{
             if(! all(Check<-sapply(object@ConcurrentTrtsDataList, 
                                    function(x) is(x,"ConcurrentTrtsData"))))
...
setMethod("addAction",signature(currentActionQ="ActionQueue",newAction="Action"),
          function(currentActionQ,newAction){
            ...
            if (NActions == 0)
              ...
              if (CurrentGlobalTimes[NActions] <= NewGlobalTime) 
                currentActionQ@ActionQ <- c(currentActionQ@ActionQ, newAction)
              else{
                IndexAfterNewAction <- min((1:NActions)[CurrentGlobalTimes > NewGlobalTime])
                currentActionQ@ActionQ[IndexAfterNewAction:(NActions+1)] <- 
                  c(newAction, currentActionQ@ActionQ[IndexAfterNewAction:NActions])
              }
            }
...
setMethod("generateBaseChar",
          ...
          function(baseCharModelSpec){  
            ...
            names(BaseChar) <- me@BaseCharName
            return(BaseChar)
          }
)
         ...
         validity=function(object){
           ...
           for ( i in 1 : length(PopModelSpec)){
             if(any(is.na(match(getRequirements(PopModelSpec[[i]]),Provisions))))
               stop("The conditioning baseline characteristics of the baseline characteristic model ",
                    paste(i)," are not generated in the previous ones of the list")
...
setMethod("generateBaseChars",signature(popModelSpec="PopModelSpecifier"),
          function(popModelSpec){ 
            BaseChars <- sapply(popModelSpec@PopModelSpec,
                                function(object) assign(object@BaseCharName, generateBaseChar(object),pos=1))
            sapply(popModelSpec@PopModelSpec,
                   function(object) rm(list=object@BaseCharName,pos=1))
            return(BaseChars)
          }
...
setGeneric("generatePatsBaseChars",
           function(popModelSpec,nPats,currentCTData) standardGeneric("generatePatsBaseChars"))
setMethod("generatePatsBaseChars",
          signature(popModelSpec="OptionalPopModelSpecifier",nPats="numeric",currentCTData="CTData"),
...
setMethod("checkRequirements",
          signature(designSpecs="list",popModelSpecs="list",
                    outcomeModelSpecs="list",
                    evalSpecs="list"),
          function(designSpecs,popModelSpecs,outcomeModelSpecs,evalSpecs){
            ...
            for ( DesignIndex in 1:length(designSpecs)){
              ...
              for ( PopMIndex in 1:length(popModelSpecs)){
                ...
                if(any(is.na(match(getRequirements(DesignSpec)$BaseChars, 
                                   getProvisions(PopModelSpec)$BaseChars)))){
                  cat("The population model ",PopMIndex,
                      " cannot provide the baseline characteristics the design ", 
                  ...
                  for(OutcomeMIndex in 1:length(outcomeModelSpecs)){
                    ...
                    if(any(
                      is.na(match(getRequirements(OutcomeModelSpec)$BaseChars, 
                                  getProvisions(PopModelSpec)$BaseChars)),
                      is.na(match(getRequirements(OutcomeModelSpec)$TrtAllos, 
                                  getProvisions(DesignSpec)$TrtAllos)),
                      is.na(match(getRequirements(DesignSpec)$Outcomes, 
                                  getProvisions(OutcomeModelSpec)$Outcomes)),
                      is.na(match(getRequirements(DesignSpec)$TimesToOutcomes, 
                                  getProvisions(OutcomeModelSpec)$TimesToOutcomes)))){
                      if(any(is.na(match(getRequirements(OutcomeModelSpec)$BaseChars, 
                                         getProvisions(PopModelSpec)$BaseChars))))
                        cat("The population model ",PopMIndex,
                            " cannot provide the baseline characteristics the outcome model ", 
                            OutcomeMIndex," requires!","\n","\n")
                      if(any(is.na(match(getRequirements(OutcomeModelSpec)$TrtAllos, 
                                         getProvisions(DesignSpec)$TrtAllos))))
                        cat("The design ",DesignIndex,
                            " cannot provide the treatment allocations the outcome model ", 
                            OutcomeMIndex," requires!","\n","\n")
                      if(any(is.na(match(getRequirements(DesignSpec)$Outcomes, 
                                         getProvisions(OutcomeModelSpec)$Outcomes))))
                        cat("The outcome model ",OutcomeMIndex,
                            " cannot provide the outcomes the design ", 
                            DesignIndex," requires!","\n","\n")
                      if(any(is.na(match(getRequirements(DesignSpec)$TimesToOutcomes, 
                                         getProvisions(OutcomeModelSpec)$TimesToOutcomes))))
                        cat("The outcome model ",OutcomeMIndex,
                            " cannot provide the times to outcomes the design ", 
                            DesignIndex," requires!","\n","\n")
                      next
                    }
                    else{
                      for(CriterionIndex in 1:length(evalSpecs)){
                        ...
                        if(any(
                          is.na(match(getRequirements(EvalSpec)$BaseChars, 
                                      getProvisions(PopModelSpec)$BaseChars)),
                          is.na(match(getRequirements(EvalSpec)$Outcomes, 
                                      getProvisions(OutcomeModelSpec)$Outcomes)),
                          is.na(match(getRequirements(EvalSpec)$CTTimes, 
                                      getProvisions(DesignSpec)$CTTimes)),
                          is.na(match(getRequirements(EvalSpec)$Conclusions, 
                                      getProvisions(DesignSpec)$Conclusions)),
                          is.na(match(getRequirements(EvalSpec)$TrtAllos, 
                                      getProvisions(DesignSpec)$TrtAllos)))){
                          if(any(is.na(match(getRequirements(EvalSpec)$BaseChars, 
                                             getProvisions(PopModelSpec)$BaseChars))))
                            cat("The population model ",PopMIndex,
                                " cannot provide the baseline characteristics the criterion ", 
                                CriterionIndex," requires!","\n","\n") 
                          if(any(is.na(match(getRequirements(EvalSpec)$Outcomes, 
                                             getProvisions(OutcomeModelSpec)$Outcomes))))
                            cat("The outcome model ",OutcomeMIndex,
                                " cannot provide the outcomes the criterion ", 
                                CriterionIndex," requires!","\n","\n")   
                          if(any(is.na(match(getRequirements(EvalSpec)$CTTimes, 
                                             getProvisions(DesignSpec)$CTTimes))))
                            cat("The design ",DesignIndex,
                                " cannot provide the CT level event times the criterion ", 
                                CriterionIndex," requires!","\n","\n") 
                          if(any(is.na(match(getRequirements(EvalSpec)$Conclusions, 
                                             getProvisions(DesignSpec)$Conclusions))))
                            cat("The design ",DesignIndex,
                                " cannot provide the conclusions the criterion ", 
                                CriterionIndex," requires!","\n","\n")   
                          if(any(is.na(match(getRequirements(EvalSpec)$TrtAllos, 
                                             getProvisions(DesignSpec)$TrtAllos))))
                            cat("The design ",DesignIndex,
                                " cannot provide the treatment allocations the criterion ", 
                                CriterionIndex," requires!","\n","\n")     
                          next
                        }
                        else ValidSetsMatrix <- rbind(ValidSetsMatrix,
                                                      c(DesignIndex,PopMIndex,OutcomeMIndex,CriterionIndex))
                      }
                    }
                  }
                }
            ...
            if(!is.null(ValidSetsMatrix)) 
              colnames(ValidSetsMatrix) <- 
              c("Design","PopM","OutcomeM","Criterion")
...
# "C:/Yuan/Research/RogerDay/Yuan/MyThesis/ExpPlatform/Data/", 
# the name of simulated CT data is in the format of
# "SimCTsData_S1.RData", where S1 refers to scenario 1
# If userInput=TRUE, then the program will ask for permission 
# to proceed after printing out the results from the
# requirements-checking.
setGeneric("doExperiment",function(designSpecs,popModelSpecs,
                                   outcomeModelSpecs,evalSpecs,nReps,seed,simDataDir,userInput) 
  standardGeneric("doExperiment"))

# If no population model is specified, then popModelSpecs=list(NULL)
setMethod("doExperiment",signature(designSpecs="list",popModelSpecs="list",
                                   outcomeModelSpecs="list",evalSpecs="list",
                                   nReps="numeric",seed="OptionalNumeric",
                                   simDataDir="OptionalCharacter",userInput="logical"),
          function(designSpecs,popModelSpecs,outcomeModelSpecs,
                   evalSpecs,nReps,seed,simDataDir,userInput){
            if(!is.null(seed)) set.seed(seed)
            # Check requirements
            ValidSetsMatrix <- checkRequirements(
              designSpecs=designSpecs,popModelSpecs=popModelSpecs,
              outcomeModelSpecs=outcomeModelSpecs,evalSpecs=evalSpecs)
            if (is.null(ValidSetsMatrix)) 
              stop("No sets of designs, population models, outcome models and evaluation criteria can work together!")
            else{
              # Check user to see if he/she wants to proceed
              NValidSets <- nrow(ValidSetsMatrix)
              # Initialize "Scenarios", 
              # each scenario corresponds to a combination of a design, a population model and an 
              # outcome model
              Scenarios <- rep(1,NValidSets)
              for ( i in 2:NValidSets){
                if(any(ValidSetsMatrix[i-1,1:3]!=ValidSetsMatrix[i,1:3])) 
                  Scenarios[i] <- Scenarios[i-1]+1
              ...
              if(answer=="No") print("Experiment is cancelled!")
                ...
                for(ScenarioIndex in 1:NScenarios){
                  ind = (Scenarios==ScenarioIndex)
                  DesignSpec <- designSpecs[[(ValidSetsMatrix[ind,"Design"][1])]]
                  PopModelSpec <- popModelSpecs[[(ValidSetsMatrix[ind,"PopM"][1])]]
                  OutcomeModelSpec <- outcomeModelSpecs[[(ValidSetsMatrix[ind,"OutcomeM"][1])]]
                  EvalSpecs <- evalSpecs[(ValidSetsMatrix[ind,"Criterion"])]
                  SimCTsData <- sapply(1:nReps,function(x) 
                    sim1CT(designSpec=DesignSpec,popModelSpec=PopModelSpec,
                           outcomeModelSpec=OutcomeModelSpec))
                  if(!is.null(simDataDir)) 
                    save(SimCTsData,
                         file=paste(simDataDir,"SimCTsData_S",ScenarioIndex,".RData",sep=""))
                  EvalOutputs[[ScenarioIndex]] <- lapply(EvalSpecs,function(x) 
                    evalDesign(evalSpec=x,simCTsData=SimCTsData))
                }
                return(EvalOutputs)
              }
